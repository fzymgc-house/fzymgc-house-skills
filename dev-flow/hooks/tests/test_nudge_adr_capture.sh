...
#
# Test harness for nudge-adr-capture hook.
# Lifted from holomush .claude/hooks/nudge-adr-capture.test.sh (PR #3833).
# Feeds synthetic PostToolUse JSON on stdin; asserts exit code + stdout.

set -uo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
HOOK="$SCRIPT_DIR/../nudge-adr-capture"
...
# --- Case 9: spec with no marker → nudge ---
