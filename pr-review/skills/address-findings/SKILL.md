...
3. If conflict: abort (`git cherry-pick --abort`), clean up (`git worktree remove
   ../<repo>_worktrees/<worktree-name>`), mark FAILED, add bead comment, re-queue.
...
2. Check for conflicts:
   ...
   Verify: `jj log -r @- --no-graph -n 1`. Clean up workspace (`jj workspace forget
   worktree-<name>` + `rm -rf ../<repo>_worktrees/<worktree-name>`). Mark FAILED, re-queue.
...
3. Update the bookmark:
   ...
   3. Clean up workspace (`jj workspace forget worktree-<name>` + `rm -rf
      ../<repo>_worktrees/<worktree-name>`). Mark FAILED, add bead comment, re-queue.
