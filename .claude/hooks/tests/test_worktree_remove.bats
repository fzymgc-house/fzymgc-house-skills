...
@test "inferred root with both .jj/ and .git/ (colocated): takes jj path, not corrupt-git skip" {
  NON_GIT=$(mktemp -d)
  mkdir -p "${NON_GIT}/.jj"
  mkdir -p "${NON_GIT}/.git"  # colocated: both present
  mkdir -p "${NON_GIT}_worktrees/colocated-inferred-wt"
  # Mock jj: 'root' exits 1 so detect_repo_root fails and triggers path inference;
  # 'workspace list' returns a line containing the workspace name;
  # 'workspace forget' logs the workspace name and exits 0.
  mkdir -p "${NON_GIT}/bin"
...
if [[ "\$1" == "workspace" && "\$2" == "list" ]]; then
  echo "worktree-colocated-inferred-wt: kkmpptqz def45678 (empty) (no description set)"
  exit 0
fi
if [[ "\$1" == "workspace" && "\$2" == "forget" ]]; then
  shift 2
...
MOCK
  chmod +x "${NON_GIT}/bin/jj"
  # Run from /tmp so detect_repo_root fails (no git/jj repo in cwd)
  PATH="${NON_GIT}/bin:/usr/bin:/bin" run bash -c 'cd /tmp && echo "{\"path\": \"'"${NON_GIT}_worktrees/colocated-inferred-wt"'\"}" | bash '"$BATS_TEST_DIRNAME"'/../worktree-remove.sh 2>&1'
  [ "$status" -eq 0 ]
  [[ "$output" == *"WARNING"* ]]
  [[ "$output" == *"inferred repo root"* ]]
  # Must NOT have skipped VCS cleanup (colocated repo should take jj path)
  [[ "$output" != *"skipping VCS cleanup"* ]]
  [ ! -d "${NON_GIT}_worktrees/colocated-inferred-wt" ]
  # jj workspace forget must have been called
  [ -f "${NON_GIT}/forget-arg.log" ]
  [[ "$(cat "${NON_GIT}/forget-arg.log")" == "worktree-colocated-inferred-wt" ]]
  rm -rf "$NON_GIT" "${NON_GIT}_worktrees"
}
