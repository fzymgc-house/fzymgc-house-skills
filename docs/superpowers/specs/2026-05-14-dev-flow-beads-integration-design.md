...
writing-plans  ◄─────────────────────┘      │
  │                                         │
  ▼                                         │
plan  ──►  plan-reviewer  ──READY──┐        │
  │             │                  │        │
  │           NOT READY            │        │
  │             │                  │        │
  └─◄───────────┘                  │        │
                                   ▼        │
                          capture-adrs (auto-fire)
                          │                 │
                          ▼                 │
                 ADRs + decision beads      │
                          │                 │
                          ▼                 │
              plan-to-beads (conditional auto: 3+ tasks)
                          │                 │
                          ▼                 │
                 epic + child beads ◄───────┘ (epic deps to decision beads)
                          │
                          ▼
...
| `plan-to-beads` | Auto if plan has 3+ tasks; offer if 1-2; skip if 0 | Skill |
| `subagent-driven-development` / `executing-plans` | User invokes | User |
| `handoff-prompt` | Manual only (cold-start session/agent for a specific bead) | User |
...
|---|---|---|
| `adr-doctor.sh` | `scripts/adr-doctor.sh` (holomush PR #3833) | Wired into `lefthook.yml` as a lint hook alongside `rumdl`. Runs 12+ doctor checks on every pre-commit (lefthook) and as a CI lint step. |

### Modified existing dev-flow skills (formerly superpowers)

| Skill | Change |
|---|---|
| `brainstorming` | At end of skill body, after spec self-review: invoke `design-reviewer`. On READY, suggest `writing-plans`. Drop "design reviewer optional" framing. |
| `writing-plans` | At end of skill body: invoke `plan-reviewer`. On READY: auto-fire `capture-adrs`; then conditional auto-fire `plan-to-beads` (3+ tasks rule). |
| `finishing-a-development-branch` | Add pre-flight check: `bd list --status=open` filtered to current epic. If any open: `AskUserQuestion` to resolve (close / file follow-up / defer). After merge succeeds (Options 1/2/4): prompt `bd close <ids>` for beads whose work merged. |
...
| `adr-doctor.sh` | Lint script | ~235 | Durable health check (12+ named checks); wired into `lefthook.yml` |
...
Six phases. Each lands as a single PR (or splits if test feedback demands).
